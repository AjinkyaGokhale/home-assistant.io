...
Ensure your energieleser device is connected to the same local network as your Home Assistant instance, and that its IP address is accessible.
...
The energieleser integration provides the following main features based on the connected device type. All measurements are provided as {% term entities %} in Home Assistant.
...
- _Monitor your energy consumption_: Add the sensors to the Home Assistant Energy dashboard to track your daily, monthly, and yearly consumption of electricity, gas, water, or heat.
- _Automate based on usage_: Trigger automations when electricity consumption goes above or below certain thresholds (e.g., turn on appliances when excess solar power is being exported).
- _Detect leaks or anomalies_: Create alerts for continuous water flow or unusual gas usage patterns to detect potential leaks.
